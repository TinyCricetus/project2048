...
export class ShapeCreator {
     ...
     */
    public creatorShape(skinStyle: number, type: number): cc.Node {
        //生产形状
        let tempNode: cc.Node = this.pool.getNode();
        //存储方块风格
        tempNode.getComponent("Grid").init(skinStyle, type);
        //配置皮肤
     ...
     * @param style 风格数组
     */
    public creatorCombineShape(style: number[], type: number): cc.Node[] {
        let tempNode: cc.Node[] = [];

        for (let i = 0; i < style.length; i++) {
            tempNode[i] = this.pool.getNode();
            tempNode[i].getComponent("Grid").init(style[i], type);
